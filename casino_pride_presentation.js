...
// ============================================================
// THEME: Professional Green — Clean, Light, Modern
// ============================================================
const T = {
  // Primary Green System
  green: "0D7C5F",       // Primary green (rich emerald)
  greenDark: "065F46",   // Darker green for headers
  greenLight: "D1FAE5",  // Light green bg
  greenMid: "34D399",    // Medium green accent
  greenSoft: "6EE7B7",   // Soft green
  greenPale: "ECFDF5",   // Pale green surface
...
s1.addShape(pres.shapes.ROUNDED_RECTANGLE, {
  x: 0.6, y: 0.5, w: 1.7, h: 0.32,
  fill: { color: T.white },
  rectRadius: 0.16,
});
s1.addText("THE CHALLENGE", {
  x: 0.6, y: 0.5, w: 1.7, h: 0.32,
  fontSize: 8, color: T.greenDark, bold: true,
...
s1.addText("Casino Pride operates as an offshore casino in Goa. Casino and gaming-related businesses fall under restricted/high-risk categories for many banks and payment gateway providers, resulting in additional scrutiny or rejection during merchant onboarding.", {
  x: 0.6, y: 3.1, w: 5.4, h: 0.85,
  fontSize: 10.5, color: T.greenLight,
...
// Right side — Proposed Approach Card
s1.addShape(pres.shapes.ROUNDED_RECTANGLE, {
  x: 6.3, y: 1.0, w: 3.3, h: 3.8,
  fill: { color: T.white },
  rectRadius: 0.15,
  shadow: { type: "outer", color: "000000", blur: 10, offset: 3, opacity: 0.15, angle: 270 },
});

// Green top accent
s1.addShape(pres.shapes.ROUNDED_RECTANGLE, {
  x: 6.6, y: 1.25, w: 1.8, h: 0.04,
  fill: { color: T.greenMid },
  rectRadius: 0.02,
});

s1.addText("Proposed Approach", {
  x: 6.6, y: 1.45, w: 2.7, h: 0.4,
  fontSize: 14, color: T.greenDark, bold: true,
  fontFace: H, align: "left", margin: 0,
});

s1.addText("Create a separate, professionally designed website with a broader focus:", {
  x: 6.6, y: 1.9, w: 2.7, h: 0.5,
  fontSize: 10, color: T.gray600,
  fontFace: B, align: "left", margin: 0,
  lineSpacingMultiple: 1.4,
});

// Focus items with green dots
const focusAreas = ["Entertainment", "Nightlife", "Hospitality", "Recreation", "Events"];
focusAreas.forEach((item, i) => {
  const y = 2.5 + i * 0.42;
  s1.addShape(pres.shapes.OVAL, {
    x: 6.6, y: y + 0.07, w: 0.14, h: 0.14,
    fill: { color: T.greenMid },
  });
  s1.addText(item, {
    x: 6.9, y: y, w: 2.4, h: 0.3,
    fontSize: 12, color: T.gray800, bold: true,
    fontFace: B, align: "left", margin: 0,
  });
});
...
s1.addText("Note: Website development cannot guarantee payment gateway approval. Final approval depends on the bank/payment gateway\u2019s KYC, compliance and risk policies.", {
  ...
  fontSize: 7.5, color: T.greenLight, italic: true,
  fontFace: B, align: "left", margin: 0,
});


// ============================================================
// SLIDE 2 — TWO POSSIBLE SOLUTIONS (Overview)
// ============================================================
...
s2.addShape(pres.shapes.ROUNDED_RECTANGLE, {
  ...
  fill: { color: T.greenPale },
  rectRadius: 0.06,
});
s2.addText("5 Pages", {
  x: 5.55, y: 4.58, w: 3.5, h: 0.2,
  fontSize: 7.5, color: T.green, bold: true,
  fontFace: B, align: "left", margin: 0,
});
...
// ============================================================
// SLIDE 3 — OPTION 1: CP Goa (Detailed)
// ============================================================
...
s3.addShape(pres.shapes.ROUNDED_RECTANGLE, {
  x: 0.5, y: 0.35, w: 1.1, h: 0.26,
  fill: { color: T.white },
  rectRadius: 0.13,
});
s3.addText("OPTION 1", {
  x: 0.5, y: 0.35, w: 1.1, h: 0.26,
  fontSize: 7.5, color: T.navyLight, bold: true,
  fontFace: B, align: "center", valign: "middle", margin: 0,
});

s3.addText("CP Goa \u2014 Entertainment & Nightlife Website", {
  x: 0.5, y: 0.75, w: 9.0, h: 0.5,
  fontSize: 24, color: T.white, bold: true,
  fontFace: H, align: "left", margin: 0,
});

s3.addText("A dedicated sub-domain website positioned around CP Goa as an entertainment brand", {
  x: 0.5, y: 1.25, w: 9.0, h: 0.3,
  fontSize: 11, color: T.greenLight,
  fontFace: B, align: "left", margin: 0,
});

// Description
s3.addText("Create a dedicated consumer-facing entertainment presence while keeping the existing cpofficial.in website separate. The website will focus on the entertainment, nightlife, and lifestyle aspects of the brand.", {
...
// Focus area pills
const pills3 = ["Entertainment", "Nightlife", "Live Events", "Dining", "Recreation", "Experiences"];
pills3.forEach((pill, i) => {
  const px = 0.5 + i * 1.55;
  s3.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    x: px, y: 2.8, w: 1.4, h: 0.35,
    fill: { color: T.navyLight },
    rectRadius: 0.17,
  });
  s3.addText(pill, {
    x: px, y: 2.8, w: 1.4, h: 0.35,
    fontSize: 9, color: T.white, bold: true,
    fontFace: B, align: "center", valign: "middle", margin: 0,
  });
});
...
// Pages card
s3.addShape(pres.shapes.ROUNDED_RECTANGLE, {
  x: 0.5, y: 3.4, w: 9.0, h: 0.55,
  fill: { color: T.gray50 },
  rectRadius: 0.08,
});
s3.addText("PROPOSED 5 PAGES:   Home   \u2022   About CP Goa   \u2022   Entertainment & Nightlife   \u2022   Experiences & Events   \u2022   Contact", {
  x: 0.7, y: 3.4, w: 8.6, h: 0.55,
  fontSize: 10, color: T.gray800, bold: true,
  fontFace: B, align: "left", valign: "middle", margin: 0,
});

// Website focus details in 2 columns
s3.addText("Website Direction", {
  x: 0.5, y: 4.15, w: 4.3, h: 0.3,
  fontSize: 11, color: T.navyLight, bold: true,
  fontFace: B, align: "left", margin: 0,
});

s3.addText("CP Goa \u2014 Entertainment \u2022 Nightlife \u2022 Experiences", {
  x: 0.5, y: 4.4, w: 4.3, h: 0.3,
  fontSize: 10.5, color: T.gray700, italic: true,
  fontFace: B, align: "left", margin: 0,
});

s3.addText("Key Benefit", {
  x: 5.2, y: 4.15, w: 4.3, h: 0.3,
  fontSize: 11, color: T.navyLight, bold: true,
  fontFace: B, align: "left", margin: 0,
});

s3.addText("Provides a dedicated consumer-facing entertainment presence while keeping the existing cpofficial.in website completely separate.", {
  x: 5.2, y: 4.4, w: 4.3, h: 0.55,
  fontSize: 10, color: T.gray700,
  fontFace: B, align: "left", margin: 0,
  lineSpacingMultiple: 1.4,
});
...
// ============================================================
// SLIDE 4 — OPTION 2: Goa Coastal Resorts (Detailed)
// ============================================================
...
s4.addText("Corporate Business Website \u2014 Broader Business Identity", {
  x: 0.5, y: 1.25, w: 9.0, h: 0.3,
  fontSize: 11, color: T.greenLight,
  fontFace: B, align: "left", margin: 0,
});
...
// Description
s4.addText("Develop a new 5-page website under the domain of Goa Coastal Resorts & Recreation Pvt. Ltd., the company associated with Casino Pride. Instead of positioning the website around casino/gaming, it presents the company\u2019s broader business identity and legitimate activities.", {
  x: 0.5, y: 2.0, w: 9.0, h: 0.65,
  fontSize: 11, color: T.gray700,
  fontFace: B, align: "left", margin: 0,
  lineSpacingMultiple: 1.5,
});
...
// Focus area pills
const pills4 = ["Hospitality", "Entertainment", "Recreation", "Dining", "Events"];
pills4.forEach((pill, i) => {
  const px = 0.5 + i * 1.9;
  s4.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    x: px, y: 2.85, w: 1.75, h: 0.38,
    fill: { color: T.green },
    rectRadius: 0.19,
  });
  s4.addText(pill, {
...
s4.addText("PROPOSED 5 PAGES:   Home   \u2022   About the Company   \u2022   Our Businesses & Experiences   \u2022   Brands / Business Interests   \u2022   Contact", {
  x: 0.7, y: 3.45, w: 8.6, h: 0.55,
  fontSize: 10, color: T.greenDark, bold: true,
  fontFace: B, align: "left", valign: "middle", margin: 0,
});

// Casino Pride note
s4.addText("Casino Pride can be presented accurately as one of the company\u2019s brands/business interests where appropriate.", {
...
s4.addShape(pres.shapes.ROUNDED_RECTANGLE, {
  ...
  fill: { color: T.greenPale },
  rectRadius: 0.08,
  line: { color: T.greenMid, width: 0.75 },
});

s4.addText("KEY ADVANTAGE", {
  x: 0.75, y: 4.68, w: 8.5, h: 0.2,
  fontSize: 8, color: T.green, bold: true,
  fontFace: B, align: "left", margin: 0,
  charSpacing: 1,
});
s4.addText("This creates a stronger corporate digital identity that can accommodate the company\u2019s existing and future business activities, rather than creating a website solely around one entertainment brand.", {
  x: 0.75, y: 4.9, w: 8.5, h: 0.45,
  fontSize: 10.5, color: T.greenDark,
  fontFace: B, align: "left", margin: 0,
...
// Features with green check marks
const features = [
  "Modern UI/UX design",
...
features.forEach((feat, i) => {
  ...
  s5.addShape(pres.shapes.OVAL, {
    x: x, y: y + 0.04, w: 0.2, h: 0.2,
    fill: { color: T.green },
  });
  s5.addText("\u2713", {
    x: x, y: y + 0.02, w: 0.2, h: 0.22,
...
s5.addText("+ GST", {
  x: 8.5, y: 2.15, w: 0.8, h: 0.3,
  fontSize: 10, color: T.greenLight,
  fontFace: B, align: "left", margin: 0,
});
...
s5.addText("Total: \u20B918,880/-", {
  x: 6.5, y: 3.05, w: 2.8, h: 0.5,
  fontSize: 16, color: T.green, bold: true,
...
s5.addShape(pres.shapes.ROUNDED_RECTANGLE, {
  ...
  fill: { color: T.white },
  rectRadius: 0.1,
  shadow: { type: "outer", color: "000000", blur: 5, offset: 2, opacity: 0.05, angle: 270 },
});
...
steps.forEach((step, i) => {
  ...
  s5.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    ...
    fill: { color: i === 0 ? T.green : T.gray100 },
    rectRadius: 0.15,
  });
  s5.addText(step, {
    x: sx, y: 4.75, w: 1.3, h: 0.3,
...
pres.writeFile({ fileName: outputPath })
  .then(() => {
    ...
    console.log("  Theme: Professional Green (Light & Modern)");
  })
  .catch(err => {
