...
def duplicate_slide(unpacked_dir: Path, source: str, after: str | None = None) -> str:
    ...
    if shared_parts:
        print(f"Note: {dest} shares its {', '.join(shared_parts)} part(s) with {source}")
    return dest
...
def _precheck_registration(unpacked_dir: Path, after: str | None, dest: str) -> str | None:
    ...
    if not pres_path.exists():
        _die(f"{pres_path} not found")
    xml = pres_path.read_text(encoding="utf-8")
    has_slot = "</p:sldIdLst>" in xml or re.search(r"<p:sldIdLst\s*/>", xml) or "</p:sldMasterIdLst>" in xml
    if not has_slot:
...
def _register_slide(unpacked_dir: Path, dest: str, source_desc: str, after_rid: str | None) -> None:
    ...
    print(f'Inserted <p:sldId id="{slide_id}" r:id="{rid}"/> at position {pos} of {total}')
...
def _get_next_slide_id(unpacked_dir: Path) -> int:
    ...
    for i in range(SLIDE_ID_MIN, SLIDE_ID_MAX + 1):
        if i not in used:
            return i
    _die("no slide id available")


def _insert_into_sld_id_lst(unpacked_dir: Path, slide_id: int, rid: str, after_rid: str | None = None) -> tuple[int, int]:
    pres_path = unpacked_dir / "ppt" / "presentation.xml"
    xml = pres_path.read_text(encoding="utf-8")
    ...
    if after_rid:
        ...
        if not open_tag:
            _die(f"{after_rid} not in <p:sldIdLst>")
        end = open_tag.end()
        if not open_tag.group(0).endswith("/>"):
            close = xml.find("</p:sldId>", end)
            if close == -1:
                _die(f"unclosed <p:sldId> for {after_rid}")
        ...
        xml = xml.replace("</p:sldMasterIdLst>", f"</p:sldMasterIdLst><p:sldIdLst>{entry}</p:sldIdLst>", 1)
    else:
        _die("no <p:sldIdLst>")
    ...
    position = next((i for i, e in enumerate(entries, 1) if f'r:id="{rid}"' in e), len(entries))
...
def add_slide_to_package(package: Path, source: str, after: str | None = None, output: Path | None = None) -> str:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Add a slide to a PPTX")
    parser.add_argument("target", help="Unpacked PPTX directory OR a .pptx/.potx file")
    parser.add_argument("source", help="slideN.xml to duplicate, or slideLayoutN.xml")
    parser.add_argument("--after", metavar="SLIDE", help="insert after this slide")
    ...
    if target.is_dir():
        if args.output:
            parser.error("--output is only valid for .pptx/.potx input")
